...
This is a provider-neutral contract, not a BGE or Cohere implementation. The PostgreSQL lexical/vector path remains unchanged until a representative benchmark shows that an authorized reranker improves quality without violating latency, privacy or offline requirements.

The selector may return more than the nominal limit when mandatory evidence itself exceeds that limit. It rejects non-positive limits rather than silently producing an invalid context.

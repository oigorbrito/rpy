...
async def _setup(monkeypatch):
    ...
    tenant_id = uuid4(); token = f"request-{uuid4()}"
    monkeypatch.setenv("RPY_BEARER_TOKENS", json.dumps({token: str(tenant_id)}))
    app.state.bearer_tokens = {token: tenant_id}; app.state.pool = pool
    async with pool.acquire() as conn:
        await conn.execute("INSERT INTO tenants(id,name) VALUES($1,'tenant')", tenant_id)
...
async def test_request_is_idempotent_and_grants_callback_process(monkeypatch):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        second=await client.post(f"/processes/{CNJ}/request",headers=headers)
        assert first.status_code == second.status_code == 202
        assert first.json()["created"] is True
        assert second.json()["created"] is False
        assert calls == 1
        payload={"event_type":"response","request_id":"req-tenant-1","response_id":"resp-tenant-1","callback_id":"cb-tenant-1","response":{"search":{"search_key":CNJ},"lawsuit":{"cnj":CNJ}}}
        monkeypatch.setenv("JUDIT_WEBHOOK_TOKEN","webhook-secret")
        callback=await client.post("/webhooks/judit/webhook-secret",json=payload)
    ...
    assert count == 1 and granted and audit == 2
    await pool.close()
...
async def test_existing_process_is_granted_without_provider_call(monkeypatch):
    ...
    async with pool.acquire() as conn:
        await conn.execute("INSERT INTO processes(code) VALUES($1)",CNJ)
    transport=httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport,base_url="http://test") as client:
        response=await client.post(f"/processes/{CNJ}/request",headers={"Authorization":f"Bearer {token}"})
    assert response.status_code == 202
    assert response.json() == {"code":CNJ,"status":"available","created":False}
    async with pool.acquire() as conn:
        assert await conn.fetchval("SELECT EXISTS(SELECT 1 FROM tenant_processes WHERE tenant_id=$1)",tenant_id)
    await pool.close()

...
async def lifespan(app: FastAPI):
    ...
    judit_webhook_max_body_bytes(); operational_thresholds(); validate_http_auth_config(); app.state.bearer_tokens=configured_bearer_tokens(); app.state.pool=await create_pool(database_url)
    try: yield
...
async def get_process_summary(code:str,request:Request)->dict:
    ...
    async with pool.acquire() as conn:
        ...
        await log_access(conn,tenant_id=tenant_id,process_id=process["id"],process_code=canonical_code,action="read_process_summary")
        summary=await conn.fetchrow("SELECT markdown, validation, model, prompt_version, generation_ms, created_at FROM process_summaries WHERE process_id=$1 AND version_id=$2",process["id"],process["current_version_id"])
    ...
    if summary_data is not None:summary_data["validation"]=decode_json_object(summary_data.get("validation"),label="summary validation")
    ...
    return {"code":process["code"],"class_name":process["class_name"],"court":process["court"],"parties":parties if isinstance(parties,list) else [],"subjects":subjects if isinstance(subjects,list) else [],"header":header if isinstance(header,dict) else {},"updated_at":process["updated_at"],"summary_status":_summary_status(summary,summary_job_status,cached_response),"summary":summary_data,"recent_steps":[dict(step) for step in steps]}

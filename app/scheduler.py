...
async def run_scheduler() -> None:
    ...
    interval_seconds = float(os.getenv("EXPUNGE_INTERVAL_SECONDS", str(timedelta(hours=24).total_seconds())))

    pool = await create_pool(database_url, min_size=1, max_size=2)
    try:
        async with pool.acquire() as lock_conn:
            if not await acquire_singleton(lock_conn):
                raise RuntimeError("another Rpy scheduler instance already holds the singleton lock")
            try:
                while True:
